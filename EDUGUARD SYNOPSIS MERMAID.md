...
# EduGuard Project Synopsis - Mermaid Edition
...
```mermaid
flowchart TD
    U[Student / Mentor / Admin] --> F[React and TypeScript Frontend]
    F -->|REST and JWT| A[ASP.NET Core Controllers]
    F -->|Real-time connection| H[SignalR EduGuard Hub]
    A --> S[Application Services]
    H --> S
    S --> M[(MongoDB)]
    S --> N[NVIDIA NIM]
    S --> E[Resend Email]
    S --> R[Report Queue Worker]
```
...
erDiagram
    COLLEGE ||--o{ ADMIN : has
    COLLEGE ||--o{ MENTOR : employs
    COLLEGE ||--o{ STUDENT : enrolls
    MENTOR ||--o{ STUDENT : guides
    STUDENT ||--o{ MESSAGE : participates
    STUDENT ||--o{ NOTIFICATION : receives
    ASSIGNMENT ||--o{ SUBMISSION : receives
    STUDENT ||--o{ SUBMISSION : creates
    STUDENT ||--o{ REPORT_JOB : requests
    STUDENT ||--o{ ATTENDANCE_RECORD : has_in_V2
    STUDENT ||--o{ LEADERSHIP_ASSIGNMENT : receives_in_V2
```
