...
export default function Layout({ user, children }: { user: User; children: ReactNode }) {
  const links = user.role === "librarian" ? [["/dashboard", "Library desk"], ["/catalog", "Catalog"]]
    : [["/admin", "Library administration"], ["/catalog", "Catalog"]];
  ...
  return <div className="shell">
    <header className="topbar"><div><img className="brand-logo" src={eduGuardLogo} alt="EduGuard" /><strong>EduGuard Library</strong></div><nav>{links.map(([path, label]) => <NavLink key={path} to={path}>{label}</NavLink>)}</nav><button className="user-button" onClick={signOut}>{user.name}<small>{user.role}</small></button></header>
    <main>{children}</main>
  </div>;
}
