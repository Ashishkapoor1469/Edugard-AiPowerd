...
export default function App() {
  ...
  const [login, setLogin] = useState({ email: "", password: "" }); const [loginError, setLoginError] = useState(""); const [signingIn, setSigningIn] = useState(false); const [showPassword, setShowPassword] = useState(false);
  useEffect(() => {
  ...
  const librarianLogin = async (event: React.FormEvent) => {
    ...
    try { const result = await api.post("/api/auth/librarian-login", { ...login, email: login.email.trim() }); sessionStorage.setItem("lmsToken", result.data.token); const me = await api.get("/api/auth/me"); setUser(me.data.data); }
