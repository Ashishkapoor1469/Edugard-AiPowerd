...
export default function StudentAchievementsSection({ student, isCr = false, canAward = false, awardedBy }: { student: StudentOption; isCr?: boolean; canAward?: boolean; awardedBy?: string }) {
  const [earned, setEarned] = useState<EarnedBadgeRecord[]>(() => readAchievementBadgeCache(student._id) || []);
  const [selected, setSelected] = useState<BadgeData | null>(null);
  const [showAward, setShowAward] = useState(false);
  const [loading, setLoading] = useState(() => readAchievementBadgeCache(student._id) === null);
  const [error, setError] = useState("");
  const load = useCallback(() => {
    setLoading(true); setError("");
    axios.get(`/api/students/${student._id}/badges`).then(({ data }) => { setEarned(data.data || []); writeAchievementBadgeCache(student._id, data.data || []); }).catch((err) => setError(err.response?.data?.message || "Could not load achievement badges")).finally(() => setLoading(false));
  }, [student._id]);
  useEffect(() => {
    if (readAchievementBadgeCache(student._id)) return;
    let active = true;
    axios.get(`/api/students/${student._id}/badges`)
      .then(({ data }) => { if (active) { setEarned(data.data || []); writeAchievementBadgeCache(student._id, data.data || []); } })
