...
export default function MyBadgesModal() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const load = () => { setError(""); return axios.get("/api/students/me/badges")
      .then((response) => {
        if (cancelled) return;
      ...
      .catch(() => { if (!cancelled) { setError("Failed to load badges."); toast.error("Failed to load badges"); } }); };
    load();
